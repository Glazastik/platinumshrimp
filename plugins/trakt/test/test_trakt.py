...
class UpdateTestCase(unittest.TestCase):
    ...
    def test_sets_last_sync_on_first_load(self):
        ...
        self.failUnless(mock_fetch.call_args_list == [call(ANY, None), call(ANY, None)])
        self.failUnless("last_sync_episodes" in self.trakt.users["adam"])
        self.assertEquals(self.trakt.users["adam"]["last_sync_episodes"], Trakt.get_date(ACTIVITY_PRESET_EPISODE_1["watched_at"]))
        self.failIf("last_sync_movies" in self.trakt.users["adam"])
        self.failIf(mock_echo.called, "No message should be sent if no last_sync had been set before")
    ...
    def test_single_episode(self, format_):
        ...
        self.failUnless(mock_echo.called, "A message should have been sent")
    ...
    def test_no_new_episodes(self):
        ...
        self.failIf(mock_echo.called, "No message should be sent if no new activities were found")
        self.assertEquals(self.trakt.users["adam"]["last_sync_episodes"], Trakt.get_date(ACTIVITY_PRESET_EPISODE_1["watched_at"]))
